...
<script>
  ...
  export default {
    ...
    methods:{
     async  getCode(){
       try {
       let sql = await  this.$store.dispatch('user/getCode',this.phone);
      ...
      async userZhu(){
        ...
        if(success){
        try {
        const {phone,code,pawd,pawds} = this;
        let password = pawd;
        phone && code && pawd==pawds && await this.$store.dispatch('user/users',{phone,password,code})
