    ...
    <section class="con">
      ...
      <div class="mainCon">
        ...
        <div class="previewWrap">
          <!--放大镜效果-->
          <Zoom :skuInfoImgList="skuInfo.skuImageList || []"/>
          <!-- 小图列表 -->
          <ImageList :skuInfoImgList="skuInfo.skuImageList || []" />
        </div>
        <!-- 右侧选择区域布局 -->
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd 
                v-for="spu in spuSaleAttr.spuSaleAttrValueList"  
                :key="spu.id"
                :class="{active : spu.isChecked == 1}"
                @click="changeAttr(spu,spuSaleAttr.spuSaleAttrValueList)"
...
<script>
  ...
  export default {
    ...
    methods:{
      changeAttr(spu,spuSaleAttrList){
        //排他行为 : 手机配置
     ...
     async  addToCart(){
          //try 成功会做什么
          try{
             // reqAddToCart是一个async函数返回的一定是一个promise对象  用 await 等待返回的结果
            let result =  await  this.$store.dispatch('detail/reqAddToCart',{skuId:this.$route.params.skuid,skuNum:this.skuNum});
           // 进行路由跳转
           // 一些简单的数据可以通过query传递，但是一些复杂的数据可以通过会话存储(不够长久，页面关闭就会消失)
